   <aside id="projects" class="col-md-2 bs-docs-sidebar">
       <nav itemscope itemtype="http://schema.org/SiteNavigationElement">
           <ul class="list-group small" itemprop="project">
                       <li><a class="list-group-item<?= (isset($_GET["period"]) ? "" : " active") ?>" href="admin.php?results">Toutes les périodes<span class="glyphicon glyphicon-th-list pull-right"></span></a></li>
                   <?php foreach($periods as $period): ?>
                       <li><a class="list-group-item<?= (!empty($_GET["period"]) ? ($_GET["period"] == $period["periode"] ? " active" : "") : "" ); ?>" href="admin.php?results=<?= trim($_GET["results"], " ")?>&amp;period=<?= trim($period["periode"], " ")?>">Période <?= $period["periode"]  ?><span class="glyphicon glyphicon-th-list pull-right"></span></a></li>
                   <?php endforeach ?>
           </ul>
       </nav>
   </aside>
           ...
           <table class="table table-hover ">
                ...
                <thead>
                  <tr>
                    ...
                    <?php endforeach ?>
                    <th class="rotate"><div><span>Moyenne</span></div></th>
                  </tr>
                </thead>
