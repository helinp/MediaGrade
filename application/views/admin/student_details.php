	...
	<div class="row chapeau">
		...
		<div class="col-xs-8  col-md-8">
			<form id="filter" method="get" class="form-inline" style="margin-top:1.5em">
                ...
                <select class="form-control input-sm" name="student" onchange="this.form.submit()">
                    <?php foreach($students as $class): ?>
						<?php foreach($class as $row): ?>
                        <?= '<option value="' . $row->id . '"' . ( @$_GET['student'] === $row->id  ? ' selected' : '') . '>' . $row->name . ' ' . $row->last_name . '</option>' . "\n" ?>
						<?php endforeach ?>
					<?php endforeach?>
                </select>
			</form>
					...
					<table class="table table-striped small">
						<?php foreach($graded as $row): ?>
							<tr>
								...
								<td><a data-toggle="modal" data-target="#projectModal" href="/admin/result_details/<?= $row->project_id?>/<?= @$_GET['student'] ?>/<?= $row->project_id?>"><span data-toggle="tooltip" data-placement="top" title="Détails" class="glyphicon glyphicon-zoom-in"> </span></a></td>
							</tr>
						<?php endforeach ?>
					</table>
				</div>
