<?php
    ...
    else
    {
        ...
        // get periods
        $periods = query("  SELECT DISTINCT periode 
                            FROM projects 
                            ORDER BY periode"
                            );
        ...
        if(!empty($_GET["class"]))
        {
            $users = query("SELECT id, last_name, name, class 
                            FROM users 
                            WHERE class = ? 
                                AND is_staff = false 
                            ORDER BY last_name",
        ...
        {
            $first_class = query("  SELECT DISTINCT class 
                                    FROM users 
                                    WHERE is_staff = 0 
                                    ORDER BY class 
                                    LIMIT 1"
        ...
        if (isset($_GET["period"]))
        {
            $projects = query(" SELECT project_name, project_id 
                                ...
                                WHERE class = ? 
                                    AND periode = ?",
                                $users[0]['class'], $_GET["period"]
                                );
        }
        
        // in case of incorrect $_GET
