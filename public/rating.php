<?php
    ...
    if (!empty($_POST["eval"]))
    {
       ...
       foreach($_POST["eval"] as $key => $eval)
       {
            ...
            if(empty(query("SELECT id FROM results WHERE user_id = ? AND project_id = ? AND skill_id = ?", $_POST["user_id"], $_POST["project"], $_POST["eval_cursor"][$key])))
            {
                query("INSERT INTO results (user_id, project_id, skill_id, user_grade) VALUES (?, ?, ?, ?)", $_POST["user_id"], $_POST["project"], $_POST["eval_cursor"][$key], $eval);
            }
            else
            {
                query("START TRANSACTION");
                    query("DELETE FROM results WHERE user_id = ? AND project_id = ? AND skill_id = ?", $_POST["user_id"], $_POST["project"], $_POST["eval_cursor"][$key]);
                    query("INSERT INTO results (user_id, project_id, skill_id, user_grade) VALUES (?, ?, ?, ?)", $_POST["user_id"], $_POST["project"], $_POST["eval_cursor"][$key], $eval);
                query("COMMIT");
            }
    ...
    elseif (!empty($_GET["rate"]) && !empty($_GET["user"]))
    {
        ...
        $users = query("SELECT users.id, name, last_name, file_path, answers
                       FROM users 
                       LEFT JOIN submitted
